...
def _run_single_query(user_query: str) -> None:
    ...
    response = final_state.get("final_response") or (
        "El sistema no generó una respuesta final. Esto no debería ocurrir; revisa los logs en modo debug."
    )

    console.print(Panel(Markdown(response), title="SGIDA", border_style="green"))
